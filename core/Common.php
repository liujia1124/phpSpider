...
class Common{
    ...
    public static $currentSpider;

    public static function getConfig($path){
        $config = include_once ROOT_PATH.$path;
        return $config;
    }
    ...
    /**
     * 获取文件夹下所有文件（不支持多层文件，自动过滤掉文件夹）
     * @param $folder
     * @param $type  路径形式   absolutely  relative  name
     * @return array         *
     */
    public static function getFolderFiles($folder,$type= 'relative'){
        ...
        if(is_dir($folder)){
            ...
            while ( ( $file = readdir ( $handle ) ) !== false )
            {
                if ( $file != '.' && $file != '..')
                {
                    switch ($type){
                       case  'absolutely';
                            $cur_path = ROOT_PATH.$folder . DIRECTORY_SEPARATOR . $file;
                            break;

                       case  'relative';
                            $cur_path = $folder . DIRECTORY_SEPARATOR . $file;
                            break;
     ...
     * @param $name
     */
    public static function setCurrentSpider($arr){
        self::$currentSpider = $arr;
    }
     ...
     * @return string
     */
    public static function getCurrentSpider(){
        return self::$currentSpider;
    }
    ...
    public static function mTimestamp()
    {
        ...
        $time = substr($time, 0, 10) . substr($time, 11, 3);
        return $time;
    }
    ...
    public static function toUtf8($data) {
        $encode_arr = array('UTF-8','GBK','GB2312');
        if (!is_array($data)) {
            if(empty($data)){
                return '';
            }
            ...
            foreach ($data as $key=>$val) {
                if(is_array($val)) {
                    ...
                    $encoded = mb_detect_encoding($val, $encode_arr);//自动判断编码
                    if($encoded == 'UTF-8'){
                        $data[$key] = $val;
                    }else{
                        $data[$key] = iconv('GBK//IGNORE', 'UTF-8', $val);
                    }
                }
     ...
     * @return string
     */
    public static function traditionToSimple($sContent){
     ...
     * @return string
     */
    public static function simpleToTradition($sContent){
        $iContent      = mb_strlen($sContent,'UTF-8');
        $traditionalCN = '';
        for($i=0;$i<$iContent;$i++){
            $str=mb_substr($sContent,$i,1,'UTF-8');
    ...
    public static function log( $path, $data){
        ...
        if(file_put_contents($path, $data.PHP_EOL, FILE_APPEND)){
            return true;
        }
        return false;
    }

    public static function htmlChar($data){
    ...
    public static function addLock($map){
        $config = self::getConfig('config/SpiderLock.php');
        $lock   = array_merge($config, $map);
        ...
        file_put_contents(ROOT_PATH.'config/spiderLock.php', $data);
    }
